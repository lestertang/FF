...
def resolve_temp_frame_set(target_path : str) -> FrameSet:
	...
	for temp_frame_path in resolve_file_pattern(temp_frame_pattern):
		frame_number = cast_int(get_file_name(temp_frame_path))
...
def get_temp_directory_path(file_path : str) -> str:
	...
	temp_file_hash = create_hash(os.path.abspath(file_path).encode())
	return os.path.join(state_manager.get_item('temp_path'), 'facefusion', temp_file_name + '-' + temp_file_hash)
