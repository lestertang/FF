...
def extract_audio_frames(spectrogram : Spectrogram, fps : Fps) -> List[AudioFrame]:
	...
	mel_filter_total = 80
	audio_step_size = 16
	indices = numpy.arange(0, spectrogram.shape[1], mel_filter_total / fps).astype(numpy.int32)
...
def get_voice_frame(audio_path : str, fps : Fps, frame_number : int = 0) -> Optional[AudioFrame]:
	if is_audio(audio_path):
		...
		if voice_frames and frame_number in range(len(voice_frames)):
...
def prepare_audio(audio : Audio) -> Audio:
	...
	audio_peak = numpy.max(numpy.abs(audio), axis = 0)

	if audio_peak > 0:
		audio = audio / audio_peak
