...
def get_writer(video_path : str, temp_video_fps : Fps, temp_video_resolution : Resolution, output_video_resolution : Resolution, output_video_fps : Fps) -> VideoWriter:
	writer_id = hashlib.sha1(str((video_path, temp_video_fps, temp_video_resolution, output_video_resolution, output_video_fps)).encode()).hexdigest()

	if writer_id not in VIDEO_POOL_SET.get('writer'):
		VIDEO_POOL_SET['writer'][writer_id] =\
		{
			'id': writer_id,
			'file_path': video_path,
	...
	return VIDEO_POOL_SET.get('writer').get(writer_id)
