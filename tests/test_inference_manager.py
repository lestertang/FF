...
def test_get_inference_pool() -> None:
	...
	model_source_set = face_classifier.get_model_options().get('sources')
	...
	with patch('facefusion.inference_manager.has_execution_provider', return_value = True):
		with patch('facefusion.inference_manager.get_onnxruntime_version', return_value = (1, 26, 0)):
			...
			with patch('facefusion.inference_manager.detect_app_context', return_value = 'cli'):
				cli_inference_pool = get_inference_pool('facefusion.face_classifier', model_names, model_source_set)

				assert isinstance(cli_inference_pool.get('fairface'), InferenceSession)

			with patch('facefusion.inference_manager.detect_app_context', return_value = 'ui'):
				ui_inference_pool = get_inference_pool('facefusion.face_classifier', model_names, model_source_set)

				assert isinstance(ui_inference_pool.get('fairface'), InferenceSession)

			assert not (cli_inference_pool.get('fairface') is ui_inference_pool.get('fairface'))
	...
	assert cli_inference_pool.get('fairface') is ui_inference_pool.get('fairface')
